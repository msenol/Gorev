...
func (h *Handlers) gorevHiyerarsiYazdir(lang string, gorev *gorev.Gorev, gorevMap map[string]*gorev.Gorev, seviye int, projeGoster bool) string {
	...
	for _, g := range gorevMap {
		if g.ParentID == gorev.ID {
			metin += h.gorevHiyerarsiYazdir(lang, g, gorevMap, seviye+1, projeGoster)
		}
	}
...
func (h *Handlers) gorevHiyerarsiYazdirVeIsaretle(lang string, gorev *gorev.Gorev, gorevMap map[string]*gorev.Gorev, seviye int, projeGoster bool, shownGorevIDs map[string]bool) string {
	// Bu görevi gösterildi olarak işaretle
	shownGorevIDs[gorev.ID] = true

	// Normal hiyerarşik yazdırma işlemi
	metin := h.gorevHiyerarsiYazdirInternal(lang, gorev, gorevMap, seviye, projeGoster, shownGorevIDs)

	return metin
}

// gorevHiyerarsiYazdirInternal görev hiyerarşisini yazdırır ve gösterilenleri işaretler
func (h *Handlers) gorevHiyerarsiYazdirInternal(lang string, gorev *gorev.Gorev, gorevMap map[string]*gorev.Gorev, seviye int, projeGoster bool, shownGorevIDs map[string]bool) string {
	indent := strings.Repeat("  ", seviye)
	prefix := ""
	if seviye > 0 {
		prefix = "└─ "
	...
	for _, g := range gorevMap {
		if g.ParentID == gorev.ID {
			...
			metin += h.gorevHiyerarsiYazdirInternal(lang, g, gorevMap, seviye+1, projeGoster, shownGorevIDs)
		}
	}
...
func (h *Handlers) GorevListele(params map[string]interface{}) (*mcp.CallToolResult, error) {
	...
	for _, kokGorev := range gorevlerToShow {
		metin += h.gorevHiyerarsiYazdirVeIsaretle(lang, kokGorev, gorevMap, 0, tumProjeler || aktifProje == nil, shownGorevIDs)
	}
...
func (h *Handlers) GorevGuncelle(params map[string]interface{}) (*mcp.CallToolResult, error) {
	...
	var updates []string
	if durum != "" {
		updates = append(updates, i18n.TLabel(lang, "durum")+": "+durum)
	}
	if oncelik != "" {
		updates = append(updates, i18n.TLabel(lang, "oncelik")+": "+oncelik)
	}
...
func (h *Handlers) GorevUstDegistir(params map[string]interface{}) (*mcp.CallToolResult, error) {
	...
	if !ok || gorevID == "" {
		return mcp.NewToolResultError(i18n.TRequiredParam(lang, "gorev_id")), nil
	}
